...
interface KeotoWebhookPayload {
  event: KeotoEventType;
  data: {
    payment_id: string;
...
export async function POST(request: NextRequest) {
  try {
    const payload: KeotoWebhookPayload = await request.json();

    console.log('Webhook Keoto recebido:', payload);
    ...
    console.error('Erro ao processar webhook Keoto:', error);
    return NextResponse.json(
...
export async function GET() {
  return NextResponse.json({
    status: 'ok',
    message: 'Webhook Keoto está funcionando',
